...
- [ ] **Step 1: Write failing contract tests.** Prove naive timestamps are
...
- [ ] **Step 2: Run RED:** `.venv/bin/pytest tests/catalysts/test_models.py tests/catalysts/test_serialization.py -q`. Expected: collection fails because `market_trader.catalysts` does not exist.
- [ ] **Step 3: Implement immutable types.** Add `AuthorityClass`, `RiskState`,
...
- [ ] **Step 4: Implement canonical serialization.** Reuse Milestone 3 canonical
...
- [ ] **Step 5: Run GREEN:** `.venv/bin/ruff check src/market_trader/catalysts tests/catalysts && .venv/bin/mypy src/market_trader/catalysts tests/catalysts && .venv/bin/pytest tests/catalysts/test_models.py tests/catalysts/test_serialization.py -q`.
- [ ] **Step 6: Commit:** `git add apps/api/src/market_trader/catalysts apps/api/tests/catalysts && git commit -m "feat: add catalyst domain contracts"`.
...
- [ ] **Step 1: Write failing tests.** Assert exact versions, fixed SEC/BLS origins,
...
- [ ] **Step 2: Run RED:** `.venv/bin/pytest tests/catalysts/test_configuration.py -q`.
- [ ] **Step 3: Add exact JSON documents.** Encode every decimal as a string and
...
- [ ] **Step 5: Run GREEN:** `.venv/bin/ruff check src/market_trader/catalysts/configuration.py tests/catalysts/test_configuration.py && .venv/bin/mypy src/market_trader/catalysts/configuration.py tests/catalysts/test_configuration.py && .venv/bin/pytest tests/catalysts/test_configuration.py -q`.
- [ ] **Step 6: Commit:** `git add apps/api/config/catalysts apps/api/src/market_trader/catalysts/configuration.py apps/api/tests/catalysts/test_configuration.py && git commit -m "feat: add versioned catalyst configuration"`.
...
- [ ] **Step 1: Write failing tests.** Define protocol fakes for company news,
...
- [ ] **Step 2: Run RED:** `.venv/bin/pytest tests/catalysts/test_providers.py tests/catalysts/test_sanitization.py -q`.
- [ ] **Step 3: Implement protocols.** Each protocol accepts an explicit immutable
  ...
  provider state.
- [ ] **Step 4: Implement `sanitize_provider_payload`.** Reuse the Milestone 3
...
- [ ] **Step 1: Write failing tests.** Cover all five event families, aware UTC,
...
- [ ] **Step 2: Run RED:** `.venv/bin/pytest tests/catalysts/test_normalizers.py -q`.
...
- [ ] **Step 4: Prove text non-authority.** Parametrize two events with identical
...
- [ ] **Step 5: Run GREEN:** `.venv/bin/ruff check src/market_trader/catalysts/normalizers.py tests/catalysts/test_normalizers.py && .venv/bin/mypy src/market_trader/catalysts/normalizers.py tests/catalysts/test_normalizers.py && .venv/bin/pytest tests/catalysts/test_normalizers.py -q`.
- [ ] **Step 6: Commit:** `git add apps/api/src/market_trader/catalysts/normalizers.py apps/api/tests/catalysts/test_normalizers.py && git commit -m "feat: normalize catalyst observations"`.
...
- [ ] **Step 1: Write failing recorded-transport tests.** Use `httpx.MockTransport`
...
- [ ] **Step 2: Run RED:** `.venv/bin/pytest tests/catalysts/adapters/test_sec.py -q`.
- [ ] **Step 3: Move `httpx>=0.28,<1` from dev-only to production dependencies.**
  Keep one declaration and regenerate no lock file because this project has none.
- [ ] **Step 4: Implement `SecEdgarAdapter`.** Inject `httpx.Client`, limiter, and
...
- [ ] **Step 5: Implement bounded retry.** Retry timeout, `429`, and retryable `5xx`
...
- [ ] **Step 6: Run GREEN:** `.venv/bin/ruff check src/market_trader/catalysts/adapters/sec.py tests/catalysts/adapters/test_sec.py && .venv/bin/mypy src/market_trader/catalysts/adapters/sec.py tests/catalysts/adapters/test_sec.py && .venv/bin/pytest tests/catalysts/adapters/test_sec.py -q`.
- [ ] **Step 7: Commit:** `git add apps/api/pyproject.toml apps/api/src/market_trader/catalysts/adapters apps/api/tests/catalysts/adapters apps/api/tests/catalysts/fixtures/http && git commit -m "feat: add official SEC catalyst adapter"`.
...
- [ ] **Step 1: Write failing recorded-transport tests.** Assert only
...
- [ ] **Step 2: Run RED:** `.venv/bin/pytest tests/catalysts/adapters/test_bls.py -q`.
- [ ] **Step 3: Add `icalendar>=7.2,<8` to production dependencies.** Use
...
- [ ] **Step 4: Implement `BlsPublicAdapter`.** Convert calendar `DTSTART` through
...
- [ ] **Step 5: Implement limits/retry.** Reuse the injected HTTP/retry primitives
...
- [ ] **Step 6: Run GREEN:** `.venv/bin/ruff check src/market_trader/catalysts/adapters/bls.py tests/catalysts/adapters/test_bls.py && .venv/bin/mypy src/market_trader/catalysts/adapters/bls.py tests/catalysts/adapters/test_bls.py && .venv/bin/pytest tests/catalysts/adapters/test_bls.py -q`.
- [ ] **Step 7: Commit:** `git add apps/api/pyproject.toml apps/api/src/market_trader/catalysts/adapters/bls.py apps/api/tests/catalysts/adapters/test_bls.py apps/api/tests/catalysts/fixtures/http && git commit -m "feat: add official BLS catalyst adapter"`.
...
- [ ] **Step 1: Write failing policy tests.** Cover earnings surprise at
...
- [ ] **Step 2: Run RED:** `.venv/bin/pytest tests/catalysts/test_classification.py -q`.
...
- [ ] **Step 4: Add vocabulary/invariant tests.** Every configured category maps
  exactly once; adding an unversioned category fails; direction never comes from
  SEC form type, BLS value alone, social, or text.
- [ ] **Step 5: Run GREEN:** `.venv/bin/ruff check src/market_trader/catalysts/classification.py tests/catalysts/test_classification.py && .venv/bin/mypy src/market_trader/catalysts/classification.py tests/catalysts/test_classification.py && .venv/bin/pytest tests/catalysts/test_classification.py -q`.
- [ ] **Step 6: Commit:** `git add apps/api/src/market_trader/catalysts/classification.py apps/api/tests/catalysts/test_classification.py && git commit -m "feat: classify structured catalyst facts"`.
...
- [ ] **Step 1: Write failing earnings tests.** Cover before-market, after-market,
...
- [ ] **Step 2: Write failing macro tests.** Cover CPI, Employment Situation, FOMC,
...
- [ ] **Step 3: Run RED:** `.venv/bin/pytest tests/catalysts/test_risk.py -q`.
- [ ] **Step 4: Implement `EventRiskEvaluator`.** Depend on the existing
...
- [ ] **Step 5: Run GREEN:** `.venv/bin/ruff check src/market_trader/catalysts/risk.py tests/catalysts/test_risk.py && .venv/bin/mypy src/market_trader/catalysts/risk.py tests/catalysts/test_risk.py && .venv/bin/pytest tests/catalysts/test_risk.py -q`.
- [ ] **Step 6: Commit:** `git add apps/api/src/market_trader/catalysts/risk.py apps/api/tests/catalysts/test_risk.py && git commit -m "feat: evaluate catalyst risk windows"`.
...
- [ ] **Step 1: Write failing tests.** Cover confirmed material directional facts,
...
- [ ] **Step 2: Run RED:** `.venv/bin/pytest tests/catalysts/test_decisions.py -q`.
- [ ] **Step 3: Implement pure decision service:**
...
- [ ] **Step 4: Add property-style invariants.** Permuting inputs cannot change
...
- [ ] **Step 5: Run GREEN:** `.venv/bin/ruff check src/market_trader/catalysts/decisions.py tests/catalysts/test_decisions.py && .venv/bin/mypy src/market_trader/catalysts/decisions.py tests/catalysts/test_decisions.py && .venv/bin/pytest tests/catalysts/test_decisions.py -q`.
- [ ] **Step 6: Commit:** `git add apps/api/src/market_trader/catalysts/decisions.py apps/api/tests/catalysts/test_decisions.py && git commit -m "feat: resolve catalyst decisions"`.
...
- [ ] **Step 1: Write failing tests.** Require summary provider ID, aware generated
  ...
  text fixtures.
- [ ] **Step 2: Run RED:** `.venv/bin/pytest tests/catalysts/test_summaries.py -q`.
- [ ] **Step 3: Implement `validate_cited_summary`.** Return accepted summary or
  typed rejection reasons. Do not import or call any model API.
- [ ] **Step 4: Prove non-authority.** Run decision and scanner-input digest tests
  before/after adding, changing, or removing summaries; every authoritative output
  must remain identical.
- [ ] **Step 5: Run GREEN:** `.venv/bin/ruff check src/market_trader/catalysts/summaries.py tests/catalysts/test_summaries.py && .venv/bin/mypy src/market_trader/catalysts/summaries.py tests/catalysts/test_summaries.py && .venv/bin/pytest tests/catalysts/test_summaries.py -q`.
- [ ] **Step 6: Commit:** `git add apps/api/src/market_trader/catalysts/summaries.py apps/api/tests/catalysts/test_summaries.py && git commit -m "feat: validate cited catalyst summaries"`.
...
- [ ] **Step 1: Write failing mapping tests.** Confirm material directional decision
...
- [ ] **Step 2: Run RED:** `.venv/bin/pytest tests/catalysts/test_scanner_adapter.py tests/scanner/test_evidence.py tests/scanner/strategies/test_news.py -q`.
- [ ] **Step 3: Implement `ScannerCatalystAdapter`.** Keep it pure and construct the
...
- [ ] **Step 4: Prove scan identity semantics.** Exact decisions preserve run key;
  changed catalyst decision/risk input changes scanner input digest/run key; prior
  persisted runs are not mutated.
- [ ] **Step 5: Run GREEN:** `.venv/bin/ruff check src/market_trader/catalysts/scanner.py src/market_trader/scanner/evidence.py tests/catalysts/test_scanner_adapter.py tests/scanner && .venv/bin/mypy src/market_trader/catalysts/scanner.py src/market_trader/scanner/evidence.py tests/catalysts/test_scanner_adapter.py tests/scanner && .venv/bin/pytest tests/catalysts/test_scanner_adapter.py tests/scanner -q`.
- [ ] **Step 6: Commit:** `git add apps/api/src/market_trader/catalysts/scanner.py apps/api/src/market_trader/scanner/evidence.py apps/api/tests/catalysts/test_scanner_adapter.py apps/api/tests/scanner && git commit -m "feat: integrate catalyst decisions with scanner"`.
...
- [ ] **Step 1: Write failing loader tests.** Assert exact fixture schema/version,
...
- [ ] **Step 2: Run RED:** `.venv/bin/pytest tests/catalysts/test_fixture_loader.py -q`.
- [ ] **Step 3: Implement loader.** Parse manifests with standard JSON and strict
...
- [ ] **Step 4: Write failing replay tests.** Use an injected virtual clock; process
...
- [ ] **Step 5: Implement replay.** Reuse production sanitizer, normalizer,
  classifier, risk evaluator, decision service, and summary validator. No replay
  branch may bypass production behavior.
- [ ] **Step 6: Run GREEN:** `.venv/bin/ruff check src/market_trader/catalysts/fixtures.py src/market_trader/catalysts/replay.py tests/catalysts/test_fixture_loader.py tests/catalysts/test_replay.py && .venv/bin/mypy src/market_trader/catalysts/fixtures.py src/market_trader/catalysts/replay.py tests/catalysts/test_fixture_loader.py tests/catalysts/test_replay.py && .venv/bin/pytest tests/catalysts/test_fixture_loader.py tests/catalysts/test_replay.py -q`.
- [ ] **Step 7: Commit:** `git add apps/api/src/market_trader/catalysts/fixtures.py apps/api/src/market_trader/catalysts/replay.py apps/api/tests/catalysts && git commit -m "feat: replay deterministic catalyst fixtures"`.
...
- [ ] **Step 1: Write failing schema tests.** Assert all five tables, columns,
...
- [ ] **Step 2: Run RED:** `.venv/bin/pytest tests/catalysts/test_schema.py tests/test_migrations.py -q`.
- [ ] **Step 3: Add ORM models.** Create `CatalystSourceRunORM`,
...
- [ ] **Step 4: Add migration.** Set `down_revision = "20260719_0003"`; create
...
- [ ] **Step 5: Run GREEN:** `.venv/bin/ruff check migrations/versions/20260719_0004_catalyst_events.py src/market_trader/db/models.py tests/catalysts/test_schema.py tests/test_migrations.py && .venv/bin/mypy src/market_trader/db/models.py tests/catalysts/test_schema.py tests/test_migrations.py && .venv/bin/pytest tests/catalysts/test_schema.py tests/test_migrations.py -q`.
- [ ] **Step 6: Commit:** `git add apps/api/migrations/versions/20260719_0004_catalyst_events.py apps/api/src/market_trader/db/models.py apps/api/tests/catalysts/test_schema.py apps/api/tests/test_migrations.py && git commit -m "feat: add catalyst persistence schema"`.
...
- [ ] **Step 1: Write failing repository tests.** Cover each create/get-by-key,
...
- [ ] **Step 2: Write failing transaction tests.** Persist a complete run; inject
...
- [ ] **Step 3: Run RED:** `.venv/bin/pytest tests/catalysts/test_repository_sink.py -q`.
- [ ] **Step 4: Implement `CatalystRepository`.** Reuse project ID/time/audit
...
- [ ] **Step 5: Implement `RepositoryCatalystSink.persist`.** Resolve symbols first;
...
- [ ] **Step 6: Run GREEN:** `.venv/bin/ruff check src/market_trader/repositories/catalysts.py src/market_trader/catalysts/sinks.py tests/catalysts/test_repository_sink.py && .venv/bin/mypy src/market_trader/repositories/catalysts.py src/market_trader/catalysts/sinks.py tests/catalysts/test_repository_sink.py && .venv/bin/pytest tests/catalysts/test_repository_sink.py -q`.
- [ ] **Step 7: Commit:** `git add apps/api/src/market_trader/repositories apps/api/src/market_trader/catalysts/sinks.py apps/api/tests/catalysts/test_repository_sink.py && git commit -m "feat: persist atomic catalyst outcomes"`.
...
- [ ] **Step 1: Write failing CLI tests.** Assert exact `validate`, `replay`, and
...
- [ ] **Step 2: Write failing conformance inventory tests.** Parametrize every
...
- [ ] **Step 3: Run RED:** `.venv/bin/pytest tests/catalysts/test_cli.py tests/catalysts/test_fixture_conformance.py -q`.
- [ ] **Step 4: Implement `main(argv: Sequence[str] | None = None) -> int`.** Resolve
...
- [ ] **Step 5: Build deterministic generator and four production groups.** Freeze
...
- [ ] **Step 1: Write failing packaging tests.** Assert the existing image copy
...
- [ ] **Step 2: Run RED:** `.venv/bin/pytest tests/test_container_configuration.py -q --no-cov`.
- [ ] **Step 3: Update smoke:** run
...
- [ ] **Step 4: Write runbook.** Cover macOS/Linux setup, offline validation/replay,
  ...
  boundaries.
- [ ] **Step 5: Run catalyst acceptance:** `.venv/bin/pytest tests/catalysts -q`.
- [ ] **Step 6: Run backend gates:** `.venv/bin/ruff check .`; strict mypy over all
...
- [ ] **Step 7: Run migration/frontend regressions:**
...
- [ ] **Step 8: Run container acceptance from repository root:**
...
- [ ] **Step 9: Mark only Milestone 5 complete.** Link specification,
  implementation plan, and runbook; set next planning action to Milestone 6; leave
  later milestone statuses unchanged.
- [ ] **Step 10: Review and commit:** `git diff --check && git status --short`, then
...
- [ ] **Step 11: Final direct review.** Invoke
...
- [ ] Every accepted observation and decision has source, timestamp, stable
  identity, policy, digest, correlation, and audit lineage.
- [ ] Fixed inputs reproduce classifications, risk windows, decisions,
  explanations, keys, and digests exactly.
- [ ] SEC/BLS adapters use only approved origins/methods/limits and fail safely.
- [ ] External text, summaries, and social-only evidence cannot confirm catalysts,
  change direction, satisfy scanner gates, alter scores, or change authoritative
  digests.
- [ ] Every approved earnings and high-impact macro boundary blocks conservatively.
- [ ] Missing, stale, malformed, unavailable, and conflicting required inputs fail
  closed with stable reasons.
- [ ] Persistence is append-only, atomic, referentially valid, and idempotent;
  identity conflicts roll back.
- [ ] Memory and persistent CLI paths render the same canonical result.
- [ ] Catalyst changes create a new scanner input identity without mutating prior
  runs.
- [ ] Ruff, new-file strict mypy, backend tests/coverage, migration, frontend,
  Docker, and offline smoke pass; inherited full-mypy debt is explicitly reported.
- [ ] User examples use `America/Chicago`; source timestamps remain UTC and exchange
